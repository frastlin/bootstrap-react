...
class DatePicker extends React.Component {
  componentDidMount() {
    ...
    var props = this.props; // Set default properties to be set if not overridden using this.props.config
    ...
    var config = {
      ...
      overrides: {
        runAfter: function (DC) {
          ...
          $A.removeClass($A.query('kbd[data-id="' + props.inputId + '"]')[0], "hidden");
          var outOfBounds = $A.isOutOfViewport(DC.outerNode);

          if (outOfBounds.bottom) {
            window.scrollBy({
              top: (window.innerHeight || document.documentElement.clientHeight) - outOfBounds.bounding.bottom,
              behavior: "smooth"
            });
          }
        },
        runAfterClose: function (DC) {
          ...
          $A.addClass($A.query('kbd[data-id="' + props.inputId + '"]')[0], "hidden");
        }
      }
