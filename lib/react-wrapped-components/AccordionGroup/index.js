...
export default class AccordionGroup extends React.Component {
  // can be used instead of importing Accordion. <AccordionGroup.Accordion {..props}>panel content</AccordionGroup.Accordion>
  constructor(props) {
    super(props);
    ...
    if (!this.props.children) {
      ...
      this.accordionElements = accordionArray.map(child => {
        ...
        this.accordionObj[`${child.props.title}-${uniqueId}`] = React.createElement(React.Fragment, null, child.props.children); // This is what is passed into the componentDidMount function into AccDC

        return React.createElement(React.Fragment, {
          key: uniqueId
        }, React.createElement(child.props.triggeringElement, {
          ...
          id: uniqueId
        }, child.props.title), React.createElement("div", {
          id: `content-${uniqueId}`
        }));
      });
    }
...
_defineProperty(AccordionGroup, "defaultProps", {
  isToggle: true,
  //says if the accordion should be toggleable
  allowMultiple: false,
  //says if more than one accordion can be expanded at once.
  callback: (DC, isOpen) => {},
  // is run every time an accordion is expanded or collapsed
  toggleClassName: "open",
  // the CSS for when the element is toggled
  className: "accordionGroup",
  // className for the accordion group
  style: {} // style for the accordion groupe

});

_defineProperty(AccordionGroup, "Accordion", AccordionElement);
