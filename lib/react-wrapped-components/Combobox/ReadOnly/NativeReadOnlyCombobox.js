...
class NativeReadOnlyCombobox extends React.Component {
  componentDidMount() {
    ...
    this.myCombobox.scrollIntoView = this.scrollIntoViewOverride; // Specify a dedicated toggle element for the Country ARIA Combobox

    this.myCombobox.setAltTrigger($A.getEl("ctryIcon")); // Add logic to process each time the toggle element state changes
    ...
    this.myCombobox.setDefault(true); // Force the highlighted value to be automatically saved when focus moves away from the Combobox

    this.myCombobox.setAutoComplete(true); // Logic to distinguish between touch screen devices

    if (!$A.isTouch()) {
      // For desktops, add screen reader accessible keyboard instructions
      this.myCombobox.setPromptText("Press the down arrow to browse available matches"); // Set a default list option display size for standard desktop screens
      ...
      this.myCombobox.setPromptText("Activate toggle to scroll through available matches"); // Set a default list option display size for touch screens, 3 for phones, 5 for tablets, 6 for others

      if ($A.device.type === "mobile") this.myCombobox.setSize(3);else if ($A.device.type === "tablet") this.myCombobox.setSize(5);else this.myCombobox.setSize(6);
    } // Optionally process after the suggestion listbox window is opened


    this.myCombobox.onOpen(function (dc) {// Do something
    }); // Process after the suggestion listbox window is closed

    this.myCombobox.onClose(function (dc) {// Do something
  ...
  scrollIntoViewOverride(optionNode, cbInstance) {
    ...
    if (cbInstance.myScroller) // optionNode is the newly highlighted role="option" node for scrolling into view.
      cbInstance.myScroller.center(optionNode);
  }

  render() {
    return React.createElement("p", null, React.createElement("label", {
      htmlFor: "ctry"
      ...
      role: "button"
    }), React.createElement(HiddenOptions, null));
  }

}

export default NativeReadOnlyCombobox;
//# sourceMappingURL=NativeReadOnlyCombobox.js.map