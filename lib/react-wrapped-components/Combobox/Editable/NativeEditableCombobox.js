...
class NativeEditableCombobox extends React.Component {
  componentDidMount() {
    ...
    this.myCombobox.scrollIntoView = this.scrollIntoViewOverride; // Disable auto population of default value

    this.myCombobox.setDefault(false); // Force the highlighted value to be automatically saved when focus moves away from the Combobox
    ...
    if (!$A.isTouch()) {
      ...
      this.myCombobox.setPromptText("First type then press the down arrow to browse available matches"); // Set a default list option display size for standard desktop screens
      ...
      this.myCombobox.setPromptText("First type then scroll through available matches"); // Set a default list option display size for touch screens, 3 for phones, 5 for tablets, 6 for others
  ...
  render() {
    return React.createElement("p", null, React.createElement("label", {
      htmlFor: "stt"
    }, "State or province:"), React.createElement("input", {
      role: "combobox",
      "aria-expanded": "false",
      type: "text",
...
//# sourceMappingURL=NativeEditableCombobox.js.map