...
export function loadAccCalendarModule() {
  if (!("setCalendar" in $A)) $A.extend({
    setCalendar: function (pId, trigger, targ, commentsEnabled, callback, config) {
      var config = config || {},
          helpTextShort = config.helpTextShort ? config.helpTextShort : "Press H for help.",
          helpText = config.helpText ? config.helpText : "Press the arrow keys to navigate by day, PageUp and PageDown to navigate by month, Alt+PageUp and Alt+PageDown to navigate by year, or Escape to cancel.",
          // Toggles for openOnFocus support.
      openOnFocusHelpText = config.openOnFocusHelpText ? config.openOnFocusHelpText : "Press Down arrow to browse the calendar, or Escape to close.",
          onFocusInit = false,
      ...
      var baseId = $A.genId(); // Calendar object declaration start

      var mainDC = $A([{
        id: pId,
        role: config.role || "Calendar",
        widgetType: "AccDCCalendar",
        ...
        RenderUsingReact: config.RenderUsingReact === true,
        allowCascade: true,
        autoPosition: isNaN(config.autoPosition) ? 0 : config.autoPosition,
        ...
        getWDay: function (dc, d, r) {
          var d = typeof d === "number" ? d : dc.range.current.wDay,
              o = dc.range.wdOffset;
          if (o < 0) d = d + o < 0 ? 7 + o : d + o;else if (o > 0) d = d + o > 6 ? -1 + (d + o - 6) : d + o;
          if (r) d = 6 - d;
          return d;
        },
        ...
        formatDate: function (dc, dateFormatTokens, dateFormat) {
          if (!dateFormatTokens) dateFormatTokens = {
        ...
        setFocus: function (o, p, s) {
          ...
          if (!s) {
            if (dc.navBtn === "PM") {
              ...
              // Toggles for openOnFocus support.
              if (!dc.openOnFocus || dc.openOnFocus && !onFocusInit && onFocusTraverse) o.focus();
            }
          }

          if (dc.fn.navBtn) dc.fn.navBtn = "";else dc.navBtn = "";
          return true;
        },
        ...
        setMonthDisabled: function (dc, dateObj, isDisabled) {
          ...
          if (isDisabled) {
            ...
            dc.range[month].disabled[year] = []; // set each day in month as disabled
        ...
        setDayOfWeekDisabled: function (dc, dateObj, daysOfWeek, isDisabled) {
          var year = dateObj.getFullYear(),
              month = dateObj.getMonth(); // initialise disabled array for month if it doesn't exist
        ...
        isDisabledDate: function (dc, counter, dateObj, cmpObj) {
          ...
          return !!(disabled && $A.inArray(counter, disabled) !== -1 || disabledAll && $A.inArray(counter, disabledAll) !== -1 || disabledWDays.length && $A.inArray(wkd, disabledWDays) !== -1 || disabledAllWDays.length && $A.inArray(wkd, disabledAllWDays) !== -1 || dc.isOutsideDateRange(dateObj));
        },
        isOutsideDateRange: function (dateObj) {
          var dateCmp = this.createDateComparisonValue(dateObj);
          return this.minDateComparisonValue && dateCmp < this.minDateComparisonValue || this.maxDateComparisonValue && dateCmp > this.maxDateComparisonValue;
        },
        createDayCell: function (i, cellDateObj, cssClasses, isDisabled, isSelected) {
          var dc = this;
          var cell = "<td "; // set correct ARIA attributes
          ...
          var comm = "";
          if (comments && comments[i]) comm = comments[i];else if (commentsAll && commentsAll[i]) comm = commentsAll[i];
          ...
          cell += '" role="link" tabindex="-1" '; // CSS classes
          ...
          var isMarked = dc.range[dc.range.current.month].marked[dc.range.current.year] && dc.range[dc.range.current.month].marked[dc.range.current.year].indexOf(i) !== -1;
        ...
        createDateComparisonValue: function (dateObj) {
          return parseInt(dateObj.getFullYear() + ("00" + dateObj.getMonth()).slice(-2) + ("00" + dateObj.getDate()).slice(-2), 10);
        },
        ...
        runBefore: function (dc) {
          ...
          dc.monthCellId = dc.baseId + "YC"; // Calculate prev/next month date values, and whether they are within the allowed date range
          ...
          nextMonth.setFullYear(nextDateValues.year); // Draw the year display and prev/next year buttons?
          ...
          if (!config.condenseYear) {
            ...
            yearSelector = '<tr class="yearSelector" role="presentation">' + '<td class="nav prev btn year' + (!hasPrevYear ? " disabled" : "") + '" accesskey="1" title="' + dc.prevTxt.replace(/<|>|\"/g, "") + " " + dc.yearTxt.replace(/<|>|\"/g, "") + '" aria-label="' + dc.prevTxt.replace(/<|>|\"/g, "") + " " + dc.yearTxt.replace(/<|>|\"/g, "") + '"' + (!hasPrevYear ? ' aria-disabled="true" tabindex="-1"' : ' tabindex="0"') + ' role="button" id="' + dc.prevBtnId + 'Y"><span aria-hidden="true">' + dc.leftButtonYearText + "</span></td>" + '<td title="' + dc.tooltipTxt.replace(/<|>|\"/g, "") + '" colspan="5" class="year" role="presentation"><span>' + dc.range.current.year + "</span></td>" + '<td class="nav next nav prev btn' + (!hasNextYear ? " disabled" : "") + '" accesskey="2" title="' + dc.nextTxt.replace(/<|>|\"/g, "") + " " + dc.yearTxt.replace(/<|>|\"/g, "") + '" aria-label="' + dc.nextTxt.replace(/<|>|\"/g, "") + " " + dc.yearTxt.replace(/<|>|\"/g, "") + '"' + (!hasNextYear ? ' aria-disabled="true" tabindex="-1"' : ' tabindex="0"') + ' role="button" id="' + dc.nextBtnId + 'Y"><span aria-hidden="true">' + dc.rightButtonYearText + "</span></td></tr>";
          ...
          var monthSelector = '<tr class="monthSelector" role="presentation">' + '<td class="nav prev btn month' + (!hasPrevMonth ? " disabled" : "") + '" accesskey="3" title="' + dc.prevTxt.replace(/<|>|\"/g, "") + " " + dc.monthTxt.replace(/<|>|\"/g, "") + '" aria-label="' + dc.prevTxt.replace(/<|>|\"/g, "") + " " + dc.monthTxt.replace(/<|>|\"/g, "") + '"' + (!hasPrevMonth ? ' aria-disabled="true" tabindex="-1"' : ' tabindex="0"') + ' role="button" id="' + dc.prevBtnId + '"><span aria-hidden="true">' + dc.leftButtonMonthText + "</span></td>" + '<td colspan="5" class="month" id="' + dc.monthCellId + '" role="presentation"><span>' + dc.range[dc.range.current.month].name + (!config.condenseYear ? "" : " " + dc.range.current.year) + "</span></td>" + '<td class="nav next btn month' + (!hasNextMonth ? " disabled" : "") + '" accesskey="4" title="' + dc.nextTxt.replace(/<|>|\"/g, "") + " " + dc.monthTxt.replace(/<|>|\"/g, "") + '" aria-label="' + dc.nextTxt.replace(/<|>|\"/g, "") + " " + dc.monthTxt.replace(/<|>|\"/g, "") + '"' + (!hasNextMonth ? ' aria-disabled="true" tabindex="-1"' : ' tabindex="0"') + ' role="button" id="' + dc.nextBtnId + '"><span aria-hidden="true">' + dc.rightButtonMonthText + "</span></td></tr>";
          dc.source = ""; // Start constructing the Datepicker table element
          // Reconfigured for Esc btn processing

          dc.source += '<table role="presentation" class="calendar">' + yearSelector + monthSelector + '<tr role="presentation">';
          dc.iter = 0; // Draw day headers
          ...
          dc.source += '</tr><tr role="presentation">'; // Start drawing day cells
          ...
          var e = m.getDay(),
              w = dc.iterS; // Draw the full calendar? (a full grid containing previous / next month cells)
          ...
          for (var i = 1; i <= dc.range[dc.range.current.month].max; i++) {
            m.setDate(i);
            var isSelected = i === dc.fn.current.mDay && dc.range.current.month === dc.fn.current.month && dc.range.current.year === dc.fn.current.year; // Draw calendar day cell
          ...
          dc.source += "</tr></table>"; // if a message is set for the month, draw it

          if (dc.range[dc.range.current.month].message[dc.range.current.year]) {
            dc.source += '<div class="monthMessage">' + "	<p>" + dc.range[dc.range.current.month].message[dc.range.current.year] + "</p>" + "</div>";
          } // Reconfigured for Esc btn processing


          if (dc.showEscBtn) {
            dc.source += '<button id="' + dc.baseId + 'esc" aria-label="' + dc.escBtnName + '" title="' + dc.escBtnName + '" class="esc-button">' + dc.escBtnIcon + "</button>";
          }
        },
        ...
        runAfter: function (dc) {
          ...
          var nMonth = function () {
            if (dc.disableNavNextMonthBtn && $A.data(dc.buttons.nM, "disabled")) return;
            $A.extend(true, dc.prevCurrent, dc.range.current);
            ...
            if (!dc.isOutsideDateRange(new Date(dateValues.year, dateValues.month, 1))) {
              var day = dc.range.current.mDay > dc.range[dateValues.month].max ? dc.range[dateValues.month].max : dc.range.current.mDay,
                  intendedDate = new Date(dateValues.year, dateValues.month, day); // If intended selected date one month ahead is outside of date range, set
              // selected date to the next available date

              if (dc.isOutsideDateRange(intendedDate)) dc.date = dc.maxDate;else dc.date = intendedDate;
            } else {
              dc.date = dc.maxDate;
            }
            ...
            dc.open();
              ...
              pMonth = function () {
            if (dc.disableNavPrevMonthBtn && $A.data(dc.buttons.pM, "disabled")) return;
            $A.extend(true, dc.prevCurrent, dc.range.current);
            var dateValues = dc.modifyDateValues({
              month: dc.range.current.month,
              year: dc.range.current.year
            ...
            if (!dc.isOutsideDateRange(new Date(dateValues.year, dateValues.month, dc.range[dateValues.month].max))) {
              var day = dc.range.current.mDay > dc.range[dateValues.month].max ? dc.range[dateValues.month].max : dc.range.current.mDay,
                  intendedDate = new Date(dateValues.year, dateValues.month, day); // If intended selected date one month previously is outside of date range, set
              // selected date to the next available date

              if (dc.isOutsideDateRange(intendedDate)) dc.date = dc.minDate;else dc.date = intendedDate;
            } else {
              dc.date = dc.minDate;
            }
            ...
            if (!forward && (!config.condenseYear && $A.data(dc.buttons.pY, "disabled") || config.condenseYear && dc.disableNavPrevYearBtn)) {
              return;
            } else if (forward && (!config.condenseYear && $A.data(dc.buttons.nY, "disabled") || config.condenseYear && dc.disableNavNextYearBtn)) {
              return;
            }
            ...
            var day = dc.range.current.mDay > dc.range[month].max ? dc.range[month].max : dc.range.current.mDay; // Only change year if the intended date is inside of any set date range
            ...
            focus: function (ev) {
              if ($A.hasClass(this, "comment")) {
                var tooltipDC = dc.children[0],
                    year = dc.range[dc.range.current.month].comments[dc.range.current.year],
                    all = dc.range[dc.range.current.month].comments["*"],
                    comm = "";
                if (year && year[dc.range.current.mDay]) comm = year[dc.range.current.mDay];else if (all && all[dc.range.current.mDay]) comm = all[dc.range.current.mDay];
              ...
              } else if (dc.children && dc.children.length && dc.children[0].loaded) {
                dc.children[0].close();
              }

              if (dc.children && dc.children.length && dc.children[1].openEditor) {
            ...
            click: function (ev) {
              ...
              if ($A.hasClass(this, "selected") || !commentsEnabled && !$A.hasClass(this, "comment")) {
                if (!$A.data(this, "disabled")) {
                  $A.extend(true, dc.fn.current, dc.range.current); // Toggles for openOnFocus support.
            ...
            keydown: function (ev) {
              ...
              if (k === 13) {
                ...
                if (k === 37) {
                  ...
                  if (dc.range.current.mDay > 1) {
                  ...
                  } else if (dc.range.current.mDay === 1 && !$A.data(dc.buttons.pM, "disabled")) {
                    var dateValues = dc.modifyDateValues({
                      month: dc.range.current.month,
                      year: dc.range.current.year
                    }, {
                    ...
                    if (dateValues.month === 1) day = new Date(dateValues.year, 1, 29).getMonth() === 1 ? 29 : 28;
                  ...
                  if (dc.range.current.mDay < dc.range[dc.range.current.month].max) {
                    ...
                    dc.setFocus(dc.range.index[dc.range.current.mDay - 1], this);
                  } else if (dc.range.current.mDay == dc.range[dc.range.current.month].max && !$A.data(dc.buttons.nM, "disabled")) {
                    var dateValues = dc.modifyDateValues({
                      month: dc.range.current.month,
                      year: dc.range.current.year
                    }, {
                  ...
                  if (dc.range.current.mDay - 7 > 0) {
                    ...
                    if (dateValues.month === 1 && new Date(dateValues.year, 1, 29).getMonth() === 1) dc.range[dateValues.month].max = 29;else if (dateValues.month === 1) dc.range[dateValues.month].max = 28;
                    var day = dc.range[dateValues.month].max + (dc.range.current.mDay - 7),
                        intendedDate = new Date(dateValues.year, dateValues.month, day); // If intended selected date one month previous is outside of date range, do not attempt
                  ...
                  if (dc.range.current.mDay + 7 <= dc.range[dc.range.current.month].max) {
                    ...
                    var day = dc.range.current.mDay + 7 - dc.range[dc.range.current.month].max,
                        intendedDate = new Date(dateValues.year, dateValues.month, day); // If intended selected date one month ahead is outside of date range, do not attempt
                    // to select the date cell
                  ...
                  // Esc key
                  dc.close(); // Toggles for openOnFocus support.
                ...
                $A.extend(true, dc.prevCurrent, dc.range.current); // Reconfigured for Esc btn processing

                if (!dc.showEscBtn) {
                  if (!config.condenseYear && !$A.data(dc.buttons.pY, "disabled")) dc.buttons.pY.focus();else if (!config.condenseYear && !$A.data(dc.buttons.nY, "disabled")) dc.buttons.nY.focus();else if (!$A.data(dc.buttons.pM, "disabled")) dc.buttons.pM.focus();else if (!$A.data(dc.buttons.nM, "disabled")) dc.buttons.nM.focus();
                  ev.preventDefault();
                }
              } else if (k === 9 && !pressed.alt && !pressed.ctrl && pressed.shift) {
                // Tab key (with simultaneous Shift modifier)
                $A.extend(true, dc.prevCurrent, dc.range.current);
                if (!$A.data(dc.buttons.nM, "disabled")) dc.buttons.nM.focus();else if (!$A.data(dc.buttons.pM, "disabled")) dc.buttons.pM.focus();else if (!config.condenseYear && !$A.data(dc.buttons.nY, "disabled")) dc.buttons.nY.focus();else if (!config.condenseYear && !$A.data(dc.buttons.pY, "disabled")) dc.buttons.pY.focus();
                ev.preventDefault();
              }
            },
            keyup: function (ev) {
          ...
          }, null, "." + baseId); // Reconfigured for Esc btn processing
          ...
          $A.on(dc.buttons.pM, {
            ...
            keydown: function (ev) {
              ...
              if (k === 13 || k === 32) {
                ...
                if (!$A.data(dc.buttons.nM, "disabled")) dc.buttons.nM.focus();else dc.query('td.day[tabindex="0"]')[0].focus();
                ev.preventDefault();
              } else if (k === 9 && !pressed.alt && !pressed.ctrl && pressed.shift) {
                if (!config.condenseYear && !$A.data(dc.buttons.nY, "disabled")) dc.buttons.nY.focus();else if (!config.condenseYear && !$A.data(dc.buttons.pY, "disabled")) dc.buttons.pY.focus();else {
                  // Reconfigured for Esc btn processing
                  if (dc.showEscBtn) dc.escBtn.focus();else dc.query('td.day[tabindex="0"]')[0].focus();
                }
                ev.preventDefault();
              }
            },
          ...
          $A.on(dc.buttons.nM, {
            ...
            keydown: function (ev) {
              ...
              if (k === 13 || k === 32) {
                ...
                if (!$A.data(dc.buttons.pM, "disabled")) dc.buttons.pM.focus();else if (!config.condenseYear && !$A.data(dc.buttons.nY, "disabled")) dc.buttons.nY.focus();else if (!config.condenseYear && !$A.data(dc.buttons.pY, "disabled")) dc.buttons.pY.focus();else {
                  // Reconfigured for Esc btn processing
                  if (dc.showEscBtn) dc.escBtn.focus();else dc.query('td.day[tabindex="0"]')[0].focus();
                }
                ev.preventDefault();
          ...
          if (!config.condenseYear) $A.on(dc.buttons.pY, {
            click: function (ev) {
              dc.navBtn = "PY";
              gYear();
              ev.preventDefault();
            ...
            keydown: function (ev) {
              ...
              if (k === 13 || k === 32) {
                ...
                if (!$A.data(dc.buttons.nY, "disabled")) dc.buttons.nY.focus();else if (!$A.data(dc.buttons.pM, "disabled")) dc.buttons.pM.focus();else if (!$A.data(dc.buttons.nM, "disabled")) dc.buttons.nM.focus();else dc.query('td.day[tabindex="0"]')[0].focus();
                ev.preventDefault();
              } else if (k === 9 && !pressed.alt && !pressed.ctrl && pressed.shift) {
                // Reconfigured for Esc btn processing
                if (dc.showEscBtn) dc.escBtn.focus();else dc.query('td.day[tabindex="0"]')[0].focus();
          ...
          if (!config.condenseYear) $A.on(dc.buttons.nY, {
            click: function (ev) {
              dc.navBtn = "NY";
              gYear(true);
              ev.preventDefault();
            ...
            keydown: function (ev) {
              ...
              if (k === 13 || k === 32) {
                ...
                if (!$A.data(dc.buttons.pM, "disabled")) dc.buttons.pM.focus();else if (!$A.data(dc.buttons.nM, "disabled")) dc.buttons.nM.focus();else dc.query('td.day[tabindex="0"]')[0].focus();
                ev.preventDefault();
              } else if (k === 9 && !pressed.alt && !pressed.ctrl && pressed.shift) {
                if (!$A.data(dc.buttons.pY, "disabled")) dc.buttons.pY.focus();else {
                  // Reconfigured for Esc btn processing
                  if (dc.showEscBtn) dc.escBtn.focus();else dc.query('td.day[tabindex="0"]')[0].focus();
                }
                ev.preventDefault();
        ...
        runAfterClose: function (dc) {
          ...
          if (dc.openOnFocus) $A.setAttr(targ, "aria-expanded", "false"); // Run custom specified function?

          if (typeof config.runAfterClose === "function") {
            config.runAfterClose(dc);
          }
        }
      }], config.overrides)[0]; // Calendar object declaration end
      ...
      var formDC = $A(mainDC, [{
        ...
        openEditor: false,
        source: '<textarea style="visibility: hidden; display: none;" class="commentTa" title="' + $A.reg.get(pId + "commentTooltip").role + '"></textarea><button title="' + (config.editor && config.editor.role || "Edit") + " " + $A.reg.get(pId + "commentTooltip").role + '" class="commentBtn">' + (config.editor && config.editor.role || "Edit") + "</button>",
        runBefore: function (dc) {
          dc.targetObj = dc.parent.outerNode;
        },
        click: function (ev, dc) {
        ...
        add: function (dc) {
          ...
          var lbl = dc.parent.range.current.mDay + ", " + dc.parent.range.wDays[dc.parent.range.current.wDay].lng + " " + dc.parent.range[dc.parent.range.current.month].name + " " + dc.parent.range.current.year,
              pre = "";
          if (dc.parent.range[dc.parent.range.current.month].disabled[dc.parent.range.current.year] && $A.inArray(dc.parent.range.current.mDay, dc.parent.range[dc.parent.range.current.month].disabled[dc.parent.range.current.year]) !== -1 || dc.parent.range[dc.parent.range.current.month].disabled["*"] && $A.inArray(dc.parent.range.current.mDay, dc.parent.range[dc.parent.range.current.month].disabled["*"]) !== -1) pre += dc.parent.disabledTxt.replace(/<|>|\"/g, "") + " ";
          if (!comm) $A.remClass(dc.parent.current, "comment");else {
        ...
        reset: function () {
          ...
          if (dc.openEditor) {
            ...
            if (!dc.textarea) dc.textarea = dc.query("textarea", function () {
              $A.css(this, {
                visibility: "",
                display: ""
              });
            ...
            $A.setAttr(dc.textarea, {
              title: dc.parent.range.current.mDay + ", " + dc.parent.range.wDays[dc.parent.range.current.wDay].lng + " " + dc.parent.range[dc.parent.range.current.month].name + " " + dc.parent.range.current.year
            }).focus();
            if (dc.comments[dc.parent.range.current.year] && dc.comments[dc.parent.range.current.year][dc.parent.range.current.mDay]) dc.textarea.value = dc.comments[dc.parent.range.current.year][dc.parent.range.current.mDay];
            $A.setAttr(dc.commentBtn, {
              title: (config.editor && config.editor.action1 || "Save") + " " + $A.reg.get(pId + "commentTooltip").role
            ...
            dc.css("left", dc.parent.outerNode.offsetLeft + dc.parent.outerNode.offsetWidth - dc.outerNode.offsetWidth);
            $A.setAttr(dc.commentBtn, {
              title: (config.editor && config.editor.role || "Edit") + " " + $A.reg.get(pId + "commentTooltip").role
        ...
        runAfter: function (dc) {
          ...
          dc.reset();
          dc.lock = true;
          $A.on(dc.query("textarea")[0], "keydown", function (ev) {
            var k = ev.which || ev.keyCode;
      ...
      }])[0]; // Form object declaration end

      $A.on(window, "resize", function (ev) {
        mainDC.setPosition();
        commentDC.setPosition();
        formDC.setPosition();
        formDC.reset();
      }, null, "." + baseId);
      $A.setAttr(trigger, "aria-expanded", "false"); // Toggles for openOnFocus support.
      ...
      var odc = $A.reg.get(pId),
        ...
        if (!odcDel && !odc.loaded && !odc.disabled) {
          odcDel = true; // Toggles for openOnFocus support.
          ...
          odcDel = true;
          odc.close(); // Toggles for openOnFocus support.
      ...
      }, null, "." + baseId); // Toggles for openOnFocus support.

      if (config.openOnFocus === true) {
        $A.setAttr(targ, "aria-expanded", "false");
        $A.on(targ, {
          touchstart: function (ev) {
            if (!odcDel && !odc.loaded && !onFocusInit && !onFocusTraverse && !odc.disabled) {
              odcDel = true;
          ...
          keydown: function (ev) {
            ...
            if (k === 40 && onFocusInit && !onFocusTraverse && odc.loaded) {
              ...
              ev.preventDefault();
              ev.stopPropagation();
            } else if (k === 40 && !odc.loaded && !odcDel && !odc.disabled) {
              odcDel = true;
              onFocusInit = true;
              onFocusTraverse = false;
              $A.trigger(trigger, "opendatepicker");
...
export default loadAccCalendarModule();
//# sourceMappingURL=Calendar.Uncompressed.js.map