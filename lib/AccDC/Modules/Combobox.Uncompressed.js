...
export function loadAccComboboxModule() {
  if (!("setCombobox" in $A)) $A.extend({
    setCombobox: function (config) {
      ...
      var isInput = ["input", "textarea"].indexOf(combobox.nodeName.toLowerCase()) !== -1;
      ...
      var options = {
        ...
        cb: {
          ...
          fn: {
            update: function () {
              ...
              dc.cb.names = [];
              dc.cb.values = [];
              dc.cb.readonly = dc.cb.isInput ? combobox.readOnly ? true : false : true;
              dc.cb.required = dc.cb.isInput ? combobox.required ? true : false : $A.getAttr(dc.triggerObj, "aria-required") === "true" ? true : false;
              ...
              for (var i = 0; i < dc.cb.optionNodes.length; i++) {
                dc.cb.baseInc++;
                var name = $A.trim($A.getText(dc.cb.optionNodes[i])).replace(/<|>/g, ""),
                    oId = dc.cb.baseId + dc.cb.baseInc,
                    o = "<" + dc.cb.childTag + ' role="option" tabindex="-1" id="' + oId + '" data-value="' + dc.cb.optionNodes[i].value + '" class="' + dc.optionClass + '" ';
                if (dc.cb.multiple) o += 'aria-checked="false" ';
                o += "><a><span>" + name + "</span></a></" + dc.cb.childTag + ">";
              ...
              if (!dc.cb.multiple) dc.cb.fn.setValue(dc.cb.options[dc.cb.optionNodes[dc.cb.sel.selectedIndex].value], true);else {
                dc.cb.autoComplete = true;
                dc.cb.fn.setValue(false, true);
              }
              if (dc.cb.required && dc.cb.isInput) $A.setAttr(dc.triggerObj, {
                "aria-required": "true"
              });
            },
            render: function (pass, scroll, noRecheck) {
              ...
              if (scroll && dc.cb.key) {
                ...
                if (!fd) dc.cb.sIndex = oI;else {
                  ...
                  if (!dc.cb.multiple) $A.setAttr(dc.triggerObj, "aria-activedescendant", dc.cb.currentOption.id);else $A.setAttr(dc.triggerObj, "aria-activedescendant", dc.cb.currentOption[0].id);
                }
                ...
                if (pass || dc.cb.showAll || dc.cb.readonly) {
                  ...
                  for (var i = 0; i < dc.cb.names.length; i++) {
                    if (dc.cb.wordMatch) {
                      var vA = $A.trim(dc.cb.value).toLowerCase().split(" "),
                          nA = $A.trim(dc.cb.names[i]).toLowerCase().split(" "),
                          vx = 0,
                          nx = 0;

                      for (var z = 0; z < vA.length; z++) {
                        for (var y = 0; y < nA.length; y++) {
                          if ($A.trim(vA[z]) && $A.trim(nA[y]) && $A.inArray($A.trim(vA[z]), $A.trim(nA[y])) !== -1 && $A.trim(vA[z]).length === $A.trim(nA[y]).length) {
                            nx++;
                            break;
                          }
                        }
                ...
                if (dc.cb.readonly) dc.cb.sIndex = dc.cb.sel.selectedIndex;else dc.cb.sIndex = 0;
            ...
            setAltTrigger: function (o) {
              ...
              if (!$A.getAccName(o).name) $A.setAttr(o, {
            ...
            setValue: function (option, pass, manual) {
              ...
              if (option && !dc.cb.multiple) {
                ...
                if (!pass && dc.cb.fn.onSelect && typeof dc.cb.fn.onSelect === "function") {
                  ...
                  if (manual || dc.cb.setDefault) dc.triggerObj.value = dc.cb.value;
                } else {
                  if (manual || dc.cb.setDefault) $A.insert(dc.cb.value, dc.cb.child);
                }
            ...
            checkValue: function (v) {
              ...
              for (var i = 0; i < dc.cb.names.length; i++) {
                if ($A.trim(v) && $A.trim(v.toLowerCase()) == $A.trim(dc.cb.names[i].toLowerCase())) {
                  return i;
                }
              }
            ...
            unsetValue: function (pass) {
              var dc = this.dc;
              if (!pass && dc.cb.sel.selectedIndex >= 0) dc.cb.optionNodes[dc.cb.sel.selectedIndex].selected = false;
              dc.cb.currentOption = null;
              ...
              $A.on(dc.triggerObj, {
                  ...
                  if (k === 9) {
                    if (dc.loaded) {
                      if (dc.cb.autoComplete && dc.cb.activeDescendant) {
                        if (!dc.cb.multiple) dc.cb.fn.setValue(dc.cb.options[dc.cb.matches[dc.cb.sIndex]], false, true);else dc.cb.fn.setValue(false, false, true);
                      }
                    ...
                    $A.setAttr(dc.cb.options[dc.cb.matches[dc.cb.sIndex]].o, "aria-checked", $A.getAttr(dc.cb.options[dc.cb.matches[dc.cb.sIndex]].o, "aria-checked") === "true" ? "false" : "true");
                    ev.preventDefault();
                  } else if (k === 38 || k === 40) {
                    ev.preventDefault();
                ...
                "keyup.AccDCCombobox": function (ev) {
                  ...
                  if (dc.cb.readonly && dc.loaded && (k === 37 || k === 39)) {
                    ...
                    dc.cb.activeDescendant = true;
                    dc.cb.sIndex = (dc.cb.readonly || dc.cb.showAll) && dc.cb.sel.selectedIndex >= 0 && !dc.cb.multiple ? dc.cb.sel.selectedIndex : 0;
                    that.open();
                    ev.preventDefault();
                  } else if ((ev.altKey && k === 40 || k === 40) && !dc.cb.activeDescendant && dc.loaded && !dc.cb.readonly) {
                    dc.cb.activeDescendant = true;
                    dc.cb.sIndex = (dc.cb.readonly || dc.cb.showAll) && dc.cb.sel.selectedIndex >= 0 && !dc.cb.multiple ? dc.cb.sel.selectedIndex : 0;
                    $A.setAttr(e, {
                      "aria-activedescendant": dc.cb.options[dc.cb.matches[dc.cb.sIndex]].id
                    });
                    $A.remClass($A.query("." + dc.activeClass, dc.source), dc.activeClass);
                    ...
                    if (dc.cb.sIndex < dc.cb.matches.length - 1) dc.cb.sIndex++;else dc.cb.sIndex = 0;
                    $A.setAttr(e, "aria-activedescendant", dc.cb.options[dc.cb.matches[dc.cb.sIndex]].id);
                    $A.remClass($A.query("." + dc.activeClass, dc.source), dc.activeClass);
                    $A.addClass(dc.cb.options[dc.cb.matches[dc.cb.sIndex]].o, dc.activeClass);
                    that.scrollIntoView(dc.cb.options[dc.cb.matches[dc.cb.sIndex]].o, that);
                    ev.preventDefault();
                    ...
                    if (dc.cb.sIndex > 0) dc.cb.sIndex--;else dc.cb.sIndex = dc.cb.matches.length - 1;
                    $A.setAttr(e, "aria-activedescendant", dc.cb.options[dc.cb.matches[dc.cb.sIndex]].id);
                    $A.remClass($A.query("." + dc.activeClass, dc.source), dc.activeClass);
                    $A.addClass(dc.cb.options[dc.cb.matches[dc.cb.sIndex]].o, dc.activeClass);
                    that.scrollIntoView(dc.cb.options[dc.cb.matches[dc.cb.sIndex]].o, that);
                    ...
                    if (dc.cb.value && x !== -1) {
                      ...
                      if (!dc.cb.multiple) dc.cb.currentOption = option;else {
                        dc.cb.currentOption = [];
                ...
                "click.AccDCCombobox": function (ev) {
                  if (!dc.cb.altTrigger) {
                    if (!dc.cb.isInput && !dc.loaded) that.open();else if ((!dc.cb.isInput || dc.cb.isInput && dc.cb.multiple) && dc.loaded) that.close();
                  }
                ...
                "blur.AccDCCombobox": function (ev) {
                  if (!$A.isTouch() && !dc.cb.multiple) {
                    setTimeout(function () {
                      if (!dc.cb.altClicked) {
                        if (dc.loaded) {
                          if (dc.cb.autoComplete && dc.cb.activeDescendant && !dc.cb.clicked) dc.cb.fn.setValue(dc.cb.options[dc.cb.matches[dc.cb.sIndex]], false, true);
                          that.close();
                        }
            ...
            set: function () {
              ...
              if (!dc.cb.isInput) {
                dc.cb.baseInc++;
                if (!dc.cb.child.id) $A.setAttr(dc.cb.child, {
                  tabindex: "-1",
            ...
            setSize: function () {
              var dc = this.dc,
                  ...
                  h = $A.elementHeight(o);
              h += parseInt($A.css(o, "margin-top"), 10) + parseInt($A.css(o, "margin-bottom"), 10);
              h = s * h + (parseInt($A.css(dc.source, "padding-top"), 10) + parseInt($A.css(dc.source, "padding-bottom"), 10));
        ...
        runBefore: function (dc) {
          ...
          for (var i = 0; i < dc.cb.matches.length; i++) {
            dc.cb.options[dc.cb.matches[i]].o = $A.toNode(dc.cb.options[dc.cb.matches[i]].source, true);
        ...
        runAfter: function (dc) {
            ...
            $A.on(dc.cb.options[v].o, {
              click: function (ev) {
                if (!dc.cb.multiple) {
                  ...
                  if (o) $A.setAttr(o, "aria-checked", $A.getAttr(o, "aria-checked") === "true" ? "false" : "true");
                  dc.cb.mClicked = true;
                }
          ...
          if (dc.cb.altTrigger && dc.cb.altTrigger.nodeType === 1) {
            ...
            if (dc.cb.fn.onTriggerChange && typeof dc.cb.fn.onTriggerChange === "function") dc.cb.fn.onTriggerChange.apply(dc.cb.altTrigger, [dc.cb.altTrigger, dc.loaded]);
          }
          ...
          if (dc.cb.fn.onOpen && typeof dc.cb.fn.onOpen === "function") dc.cb.fn.onOpen.apply(dc.triggerObj, [dc]);
        },
        ...
        runAfterClose: function (dc) {
          ...
          $A.off("body", "." + baseId);
          if (dc.cb.fn.onClose && typeof dc.cb.fn.onClose === "function") dc.cb.fn.onClose.apply(dc.triggerObj, [dc]);
        },
      ...
      that.clearAll = function () {
        ...
        if (dc.cb.isInput) dc.triggerObj.value = "";else if (dc.cb.child) {
          if (!dc.cb.multiple) $A.empty(dc.cb.child);else dc.cb.fn.setValue(false, true);
        }
      };
      ...
      that.scrollIntoView = function (o, cb) {
        ...
        o.scrollIntoView();
        if (document.documentElement.scrollLeft !== i) document.documentElement.scrollLeft = i;
      };

      that.getValue = function () {
        if (!dc.cb.multiple) return dc.cb.sel.value;else {
          var r = [];
      ...
      if (!dc.cb.isInput && !$A.getAttr(dc.triggerObj, "tabindex")) $A.setAttr(dc.triggerObj, "tabindex", "0");
      dc.cb.fn.set();
      return that;
    }
  });
...
//# sourceMappingURL=Combobox.Uncompressed.js.map