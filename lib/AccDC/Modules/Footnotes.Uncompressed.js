  ...
  if (!("setFootnotes" in $A)) $A.extend({
    setFootnotes: function (overrides) {
      ...
      $A.query(selector, context, function (i, o) {
        ...
        if ($A.setFootnotes[base][base + fnId + i] && $A.setFootnotes[base][base + fnId + i].nodeType === 1 && $A.setFootnotes[base][base + fnId + i].parentNode) {
          $A.remove($A.setFootnotes[base][base + fnId + i]);
          delete $A.setFootnotes[base][base + fnId + i];
          $A.data(o, "BoundFootnote", false);
        }

        if ($A.setFootnotes[base + "a"][base + fnId + i] && $A.setFootnotes[base + "a"][base + fnId + i].nodeType === 1 && $A.setFootnotes[base + "a"][base + fnId + i].parentNode) {
...
//# sourceMappingURL=Footnotes.Uncompressed.js.map