...
window.getAccName = window.calcNames = function (node, fnc, preventVisualARIASelfCSSRef) {
  var props = {
    name: "",
    desc: "",
    error: ""
  ...
  try {
    ...
    var rootNode = node; // Track nodes to prevent duplicate node reference parsing.
    ...
    var walk = function (refNode, stop, skip, nodesToIgnoreValues, skipAbort, ownedBy) {
      ...
      */
      ...
      var isException = function (node, refNode) {
        ...
        else if (inList(node, list2) || node === rootNode && !inList(node, list1)) {
            return true;
          } else {
            return false;
          }
      ...
      fullResult = walkDOM(refNode, function (node) {
        ...
        var isEmbeddedNode = node && node.nodeType === 1 && nodesToIgnoreValues && nodesToIgnoreValues.length && nodesToIgnoreValues.indexOf(node) !== -1 && node === rootNode && node !== refNode ? true : false;
        ...
        if (nodes.indexOf(parent) === -1) {
          nodes.push(parent); // Store the before and after pseudo element 'content' values for the current node container element
          // Note: If the pseudo element includes block level styling, a space will be added, otherwise inline is asumed and no spacing is added.
        ...
        if (node.nodeType === 1) {
          ...
          if (!stop && node === refNode) {
            ...
            if (aLabelledby) {
              ...
              for (i = 0; i < ids.length; i++) {
                element = document.getElementById(ids[i]); // Also prevent the current form field from having its value included in the naming computation if nested as a child of label
              ...
              if (trim(name)) {
                hasName = true; // Abort further recursion if name is valid.
            ...
            if (aDescribedby) {
              ...
              for (i = 0; i < ids.length; i++) {
                ...
                parts.push(walk(element, true, false, [node], false, {
                  ref: ownedBy,
                  top: element
                }).name);
              } // Check for blank value, since whitespace chars alone are not valid as a name
          ...
          if (!hasName && trim(aLabel) && !isSeparatChildFormField) {
            name = aLabel; // Check for blank value, since whitespace chars alone are not valid as a name
          ...
          if (!hasName && node === refNode && isNativeFormField) {
            ...
            if (explicitLabel) {
              var eLblName = trim(walk(explicitLabel, true, skip, [node], false, {
                ref: ownedBy,
                top: explicitLabel
              }).name);
            }
            ...
            if (implicitLabel && implicitLabel !== explicitLabel) {
              var iLblName = trim(walk(implicitLabel, true, skip, [node], false, {
                ref: ownedBy,
                top: implicitLabel
              }).name);
            }
          ...
          var nAlt = rolePresentation ? "" : trim(node.getAttribute("alt")); // Otherwise, if name is still empty and current node is a standard non-presentational img or image button with a non-empty alt attribute, set alt attribute value as the accessible name.

          if (!hasName && !rolePresentation && (nTag === "img" || btnType === "image") && nAlt) {
            // Check for blank value, since whitespace chars alone are not valid as a name
            name = trim(nAlt);
          ...
          if (!hasName && node === refNode && btnType && ["button", "image", "submit", "reset"].indexOf(btnType) !== -1) {
            if (btnValue) {
              name = btnValue;
            } else {
              switch (btnType) {
          ...
          if (hasName && node === refNode && btnType && ["button", "submit", "reset"].indexOf(btnType) !== -1 && btnValue && btnValue !== name && !result.desc) {
          ...
          if (node === rootNode && !rolePresentation && trim(nTitle) && !isSeparatChildFormField) {
          ...
          if (aOwns && !isNativeFormField && nTag !== "img") {
            ...
            for (i = 0; i < ids.length; i++) {
              element = document.getElementById(ids[i]); // Abort processing if the referenced node has already been traversed
        ...
        result.owns = ariaO;
        return result;
      }, refNode); // Prepend and append the refObj CSS pseudo element text, plus normalize whitespace chars into flat spaces.
    ...
    var isFocusable = function (node) {
      ...
      if (["button", "input", "select", "textarea"].indexOf(nodeName) !== -1 && node.getAttribute("type") !== "hidden") {
        return true;
      }
    ...
    var list2 = {
      roles: ["application", "alert", "log", "marquee", "timer", "alertdialog", "dialog", "banner", "complementary", "form", "main", "navigation", "region", "search", "article", "document", "feed", "figure", "img", "math", "toolbar", "menu", "menubar", "grid", "listbox", "radiogroup", "textbox", "searchbox", "spinbutton", "scrollbar", "slider", "tablist", "tabpanel", "tree", "treegrid", "separator", "rowgroup"],
      tags: ["article", "aside", "body", "select", "datalist", "optgroup", "dialog", "figure", "footer", "form", "header", "hr", "img", "textarea", "input", "main", "math", "menu", "nav", "section", "thead", "tbody", "tfoot"]
    ...
    var list3 = {
      roles: ["term", "definition", "directory", "list", "group", "note", "status", "table", "contentinfo"],
    ...
    var isBlockLevelElement = function (node, cssObj) {
      ...
      for (var prop in blockStyles) {
        ...
        for (var i = 0; i < values.length; i++) {
          if (styleObject[prop] && (values[i].indexOf("!") === 0 && [values[i].slice(1), "inherit", "initial", "unset"].indexOf(styleObject[prop]) === -1 || styleObject[prop].indexOf(values[i]) === 0)) {
            return true;
          }
        }
      }

      if (!cssObj && node.nodeName && blockElements.indexOf(node.nodeName.toLowerCase()) !== -1 && !(styleObject["display"] && styleObject["display"].indexOf("inline") === 0 && node.nodeName.toLowerCase() !== "br")) {
        return true;
      }
    ...
    var joinSelectedParts = function (node, nOA, isNative, childRoles) {
      ...
      for (var i = 0; i < nOA.length; i++) {
        ...
        if (isValidChildRole) {
          parts.push(isNative ? getText(nOA[i]) : walk(nOA[i], true, false, [], false, {
            top: nOA[i]
          }).name);
        }
      }
    ...
    var accProps = walk(node, false, false, [], false, {
      top: node
    });
    ...
    props.name = accName;
    props.desc = accDesc; // Clear track variables
...
window.getAccNameMsg = window.getNames = function (node) {
  ...
  if (props.error) {
    ...
    return 'accName: "' + props.name + '"\n\naccDesc: "' + props.desc + '"\n\n(Running AccName Computation Prototype version: ' + window.getAccNameVersion + ")";
  }
};
